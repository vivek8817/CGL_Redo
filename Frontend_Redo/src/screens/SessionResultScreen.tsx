...
import { JoyfulFace, SensitiveFace } from '../components/Faces';

const SessionResultScreen = () => {
  const navigate = useNavigate();
  const location = useLocation();

  // Pull result data from location state, or fallback if accessed directly
  const state = location.state as { score: number; wrong: number; total: number } | null;
    ...
    <div className="flex-1 w-full bg-background-app flex flex-col relative shrink-0 h-full">
      ...
      <div className="bg-surface rounded-t-overlay rounded-b-sm px-card-pad pt-16 pb-8 z-10 flex flex-col items-center relative shrink-0 shadow-sm text-center">
        ...
        {isGood ? (
          ...
          <SensitiveFace size={120} className="mb-6 drop-shadow-md" />
        )}

        <h1 className="font-display text-4xl font-bold tracking-tight text-text-primary leading-none mb-2">
          {isGood ? "Awesome Session!" : "Keep Practicing!"}
        </h1>
        <p className="text-sm font-bold text-text-muted">
          You scored <span className="text-text-primary">{result.score}</span> out of {result.total}.
        </p>
      ...
      <main className="flex-1 overflow-y-auto pt-4 pb-24 flex flex-col gap-layout-gap scrollbar-hide px-card-pad">
        ...
        <div className="grid grid-cols-2 gap-layout-gap mb-4">
          <div className="bg-widget-quiz-bg/20 border border-widget-quiz-bg/30 rounded-sm p-card-pad flex flex-col items-center justify-center gap-1">
             <span className="text-3xl font-display font-bold text-[#0f5132] leading-none">{result.score}</span>
        ...
        <div className="flex flex-col gap-4 mt-auto">
          {result.wrong > 0 && (
            <button 
              onClick={() => navigate(-1)} // Currently just going back, but in future would trigger Revise Wrong
