...
const CalendarScreen = () => {
  ...
  const activityLog = useSelector(
    (state: RootState) => state.streak.activityLog || {},
  );
  ...
  while (true) {
    ...
    if (log && log.questionsAttempted >= 20) {
      ...
      // Break the streak, UNLESS it's exactly today and they just haven't started yet
      const isTodayStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
      if (dateStr !== isTodayStr || (log && log.questionsAttempted > 0)) {
        break; // Streak broken
      }
    }
    
    // Step backwards exactly one day (JS automatically handles month/year rollovers!)
  ...
  const renderFace = (dateStr: string, isFuture: boolean) => {
    if (isFuture)
      return (
        <span className="text-text-muted opacity-40 font-bold text-md cursor-default">
          ---
        </span>
      );
    const log = activityLog[dateStr];

    if (!log || log.questionsAttempted === 0)
      return (
        <AngryFace className="cursor-pointer active:scale-95 transition-transform" />
      );
    if (log.questionsAttempted < 20)
      return (
        <NeutralFace className="cursor-pointer active:scale-95 transition-transform" />
      );
    return (
      <HappyFace className="cursor-pointer active:scale-95 transition-transform" />
    );
  };
      ...
      <div className="bg-surface rounded-t-overlay rounded-b-sm px-card-pad pt-12 pb-8 z-10 flex flex-col relative shrink-0 shadow-sm">
        ...
        <div className="grid grid-cols-7 mb-4">
          {["M", "T", "W", "T", "F", "S", "S"].map((day, i) => (
            <div
              key={i}
              className="text-center text-xs font-bold text-text-muted uppercase tracking-wider"
            >
              {day}
            </div>
          ))}
          ...
          {daysArray.map(({ date, dateStr, isFuture }) => {
            ...
            const accuracy =
              log && log.questionsAttempted > 0
                ? Math.round(
                    (log.correctAnswers / log.questionsAttempted) * 100,
                  )
                : 0;
              ...
              <Popover.Root key={dateStr}>
                <Popover.Trigger asChild>
                  <div className="flex flex-col items-center gap-1 outline-none">
                    ...
                    <span className="text-[10px] font-bold text-text-muted opacity-80">
                      {date.getDate()}
                    </span>
                  </div>
                </Popover.Trigger>
                        ...
                        {log && log.questionsAttempted > 0 ? (
                          <>
                            ...
                            <div className="flex justify-between items-center">
                              ...
                              <span
                                className={`text-sm font-bold ${accuracy >= 70 ? "text-[#0f5132]" : "text-widget-sleep-chart"}`}
                              >
                                {accuracy}%
                              </span>
                            </div>
      ...
      <main className="flex-1 overflow-y-auto pt-4 pb-24 flex flex-col gap-layout-gap scrollbar-hide px-layout-gap">
        ...
        <div className="bg-widget-quiz-bg rounded-sm p-card-pad flex justify-between items-center shadow-sm relative overflow-hidden">
          <div className="flex flex-col z-10">
            <span className="text-xs font-bold text-text-primary opacity-80 uppercase tracking-wider mb-1">
              Current Streak
            </span>
            ...
            {currentStreak === 0 && (
              <span className="text-xs font-bold text-text-primary opacity-70 mt-1">
                Hit 20 questions to start a streak!
              </span>
            )}
            {currentStreak > 0 && (
              <span className="text-xs font-bold text-text-primary opacity-70 mt-1">
          ...
          <iconify-icon
            ...
            width="80"
            className="absolute -right-4 -bottom-4 opacity-20 text-text-primary"
          ></iconify-icon>
        </div>
        ...
        <div className="grid grid-cols-2 gap-layout-gap">
          ...
          <div className="bg-surface-hover rounded-sm p-card-pad flex flex-col gap-1 border border-border/50">
            <iconify-icon
              ...
              className="mb-1"
            ></iconify-icon>
            <span className="text-base font-bold text-text-primary leading-tight mt-1 line-clamp-2 min-h-[40px]">
              {topSubject}
            </span>
            <span className="text-xs font-bold text-text-muted uppercase tracking-wider">
              Top Subject
